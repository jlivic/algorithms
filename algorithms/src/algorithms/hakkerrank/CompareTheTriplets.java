 ...
 * 
 * If a[i] > b[i], then Alice is awarded 1 point. If a[i] < b[i], then Bob is
 * awarded 1 point. If a[i] = b[i], then neither person receives a point.
 ...
 * Example
 * 
 * a = [1, 2, 3] b = [3, 2, 1] For elements *0*, Bob is awarded a point because
 * a[0] . For the equal elements a[1] and b[1], no points are earned. Finally,
 * for elements 2, a[2] > b[2] so Alice receives a point. The return array is
 ...
 * 
 * int[2]: Alice's score is in the first position, and Bob's score is in the
 * second. Input Format
 * 
 ...
 * Explanation 1:
 * 
 * Comparing the elements, so Bob receives a point. Comparing the and elements,
 * and so Alice receives two points. The return array is [2,1].
 * 
 * @author jlivic
 *
 */
